...
export default async function ImpactPage(){
  ...
  const [{count:disasterCount},{count:activePrograms},{count:urgentPrograms},{data:countyRows}] = await Promise.all([
    ...
    supabase.from('counties').select('name,precipitation_deficit_inches').order('precipitation_deficit_inches',{ascending:false}).limit(12),
  ])
  const chartData=(countyRows??[]).map((r)=>({county:r.name,deficit:Number(r.precipitation_deficit_inches??0)}))
    ...
    <main className={styles.container}>
      ...
      <div className={styles.inner}>
        <header className={`${styles.header} animate-on-scroll`}>
          ...
          <p className={styles.headerText}>Mission-critical metrics synced from current county and program tables.</p>
        </header>
        ...
        <section className={`${styles.content} animate-on-scroll`}>
          ...
          <DataChart data={chartData} xAxisKey="county" yAxisKey="deficit" type="bar" title="Top Rainfall Deficits (inches)" color="var(--color-harvest)" height={360} />
        </section>
        ...
        <section className={`${styles.footerCard} animate-on-scroll`}>
          ...
          <p className={styles.footerText}>Use the live program list to turn county stress indicators into immediate filing actions.</p>
